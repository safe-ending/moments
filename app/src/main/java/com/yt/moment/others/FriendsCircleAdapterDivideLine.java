...
public class FriendsCircleAdapterDivideLine extends RecyclerView.ItemDecoration {
    private float mDividerHeight = Utils.dp2px(8); //线的高度
    ...
    public FriendsCircleAdapterDivideLine() {
        ...
        mPaint.setColor(Color.parseColor("#F5F6FA"));        //默认颜色
    }

    //通过建造者模式来设置三个属性
    //设置左右偏移(默认是设置的一样的，若需要自己更改)
    public FriendsCircleAdapterDivideLine setMargin(float margin) {
        this.margin = margin;
        return this;
    }

    //设置颜色
    public FriendsCircleAdapterDivideLine setColor(int color) {
        mPaint.setColor(color);
        return this;
    }

    //设置分割线高度
    public FriendsCircleAdapterDivideLine setDividerHeight(float height) {
        this.mDividerHeight = Utils.dp2px(height);
        return this;
    }
